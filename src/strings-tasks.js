...
function sumOfCodes(str) {
  ...
  return chars.reduce((acc, char) => {
    const newAcc = acc + char.charCodeAt(0);
    return newAcc;
  }, 0);
}
...
function formatTime(minutes, seconds) {
  ...
  return `${normalizeValue(minutes)}:${normalizeValue(seconds)}`;
}
...
function invertCase(str) {
  return str
    ...
    .reduce((acc, currentWord) => {
      const [...chars] = currentWord;
      const invertCaseStr = chars
        .map((char) =>
          char === char.toLowerCase() ? char.toUpperCase() : char.toLowerCase()
        )
        .join('');
      const newAcc = [...acc, invertCaseStr];
      return newAcc;
    }, [])
    .join(' ');
}
...
function extractNameFromTemplate(value) {
  ...
  const name = value.substring(commaIndex + 2, value.length - 1).trim();
  return name;
}
